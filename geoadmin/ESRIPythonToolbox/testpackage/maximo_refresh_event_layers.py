...
def pytmain(params):
    ...
    try:
        ...
        output_dir = params[0].valueAsText
        route = params[1].valueAsText
        arcpy.env.scratchWorkspace = output_dir
        desc = arcpy.Describe(route)
        route1, output_dir1 = manage_runtime_params(route, output_dir)

        #tool zero
        #call_stored_proc()
        
        #tool one
        refresh_events(route1, output_dir1)

        #tool two
        refreshed_data_dict = build_source_destination_dict(route1, output_dir1)
        manage_persist_data(refreshed_data_dict)
...
def manage_runtime_params(route, output_dir):
    if get_arcpy_runtime() == "python":
        output_dir1 = r"O:\AppData\GIS\Cloud\Users\JPerez\workspace\20141113_RouteEvents"
        route1 = r"Database Connections\GISCLOUDDEV_os_pods_oms.sde\pods_oms.GIS.Transmission\pods_oms.GIS.StationSeries"
